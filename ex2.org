...
Veremos cómo se resuelve este problema usando la biblioteca =pulp= se Python
...
5. Un fabricante produce dos tipos de ropa: \(T_{1}\) y
   ...
   Si la ganancia de la venta de una unidad de \(T_{1}\) es de 4 pesos y la
   de una unidad de \(T_{2}\) es 5 pesos, ¿cuánto se debe producir de
...
6. Una compañía que produce frutas mezcladas tiene en almacén 10,000
   ...
   cada combinación son de 3, 4 y 5 pesos respectivamente. ¿Cuántas
